...
def train(hyp, tb_writer, opt, device):
    ...
    log_dir = tb_writer.log_dir if tb_writer else os.path.join(opt.outdir, 'evolution')  # run directory
...
if __name__ == '__main__':
    ...
    parser.add_argument('--outdir', type=str, default='runs', help='Storage path of generated products')
    ...
    if not opt.evolve:
        if opt.local_rank in [-1, 0]:
            print(f'Start Tensorboard with "tensorboard --logdir={opt.outdir}", view at http://localhost:6006/')
            tb_writer = SummaryWriter(log_dir=increment_dir(os.path.join(opt.outdir,'exp'), opt.name))
        else:
            tb_writer = None
